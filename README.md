<header>
<!--  <<< Portfolio Name >>> -->

# Amy Wallace - Portfolio

## About Me
👩🏻‍💻I'm a passionate analyst who aims to use my skillset to bring purpose and change, bringing the best out of people, systems, and processes. 

🔍I am currently looking for analyst roles in sport, government, or intelligence where I can bring real purpose to the world.

I am currently working on:<br>
🏃‍♀️ My Womens Sports Analytix Project.<br>
🌐 Upgrading Albert Park Women's Golf Club's Webpage.<br>
</header>

<!-- -->


## 👩‍🎓 Education
**Graduate Certificate**- Business Analytics
September 2022 - July 2023
<br><br>


## 🔧 Technical Skills  

### 🖥️ Programming Languages  
* SQL  
* HTML
* CSS
* R
* Python (Beginner)


### 📊 Data Visualisation  
* Power BI
* Tableau
* SAS Viya
* Excel

### 🛠 Applications 
* Jira
* GitHub
* Microsoft Visual Studio Code
* Microsoft365
* LucidChart
* Canva
* CVAT
* Boxy-SVG 


### 📈 Models & Techniques 
🔹 **Models**: Descriptive, Predictive, Comparitive, Relational, Clustering, Anomaly, Optimisation, Process Mapping, Gap Analysis, Kanban, Waterfall <br>
🔹 **Techniques**: Visualisations, Story Telling, Linear Regression, Time Series, Outlier Detection, Flowcharts, MOSCOW, Burn Down & Gantt Charts, Image Annotation
<br><br>
## 📊 Data Analysis Projects <br> 
### 📌 Descriptive Analysis  

**Objective**: Analyse demographic, economic, and infrastructure data to uncover patterns affecting community well-being and public service utilisation. 
  Provide insights to inform policy decisions, resource allocation, and infrastructure improvements using descriptive analytics.<br>
**Tools**: Visual Studio Code, Python, Excel, Power BI<br>
**Methods**: Descriptive Analytics, Regression Model, Correlation & Variable Relationships, Geospatial & Demographic analysis<br>

**🔗 [View Project](https://github.com/WallaceAmy/Descriptive---Community-Government-Project/blob/main/README.md#descriptive---community-government-project)**<br><br>

<br>


<!-- -->
<footer>
</footer>
